...
|-----------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [IBM Cloud](https://cloud.ibm.com)      | [IBM Cloud Quick Start](https://github.com/IBM/automation-ibmcloud-infra-openshift) <br /> [IBM Cloud Standard](https://github.com/IBM/automation-ibmcloud-infra-openshift) <br /> [IBM Cloud Advanced - Coming soon](https://github.com/IBM/automation-ibmcloud-infra-openshift) |  
| [AWS](https://aws.amazon.com/)          | [AWS Quick Start](https://github.com/IBM/automation-aws-infra-openshift) <br /> [AWS Standard](https://github.com/IBM/automation-aws-infra-openshift) <br /> [AWS Advanced - Coming soon](https://github.com/IBM/automation-aws-infra-openshift) |
| [Azure](https://portal.azure.com/#home) | [Azure Quick Start](https://github.com/IBM/automation-azure-infra-openshift) <br /> [Azure Standard](https://github.com/IBM/automation-azure-infra-openshift) <br /> [Azure Advanced - Coming soon](https://github.com/IBM/automation-azure-infra-openshift) |                                                                                             | 
...
7. Download the `license.dat` file that was generated on the next page and save this.  The path to this file can be provided as input along with the host id to automatically apply the license.
...
|--------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|
| 105    | [105 - Existing OpenShift](./105-existing-openshift) (or other OpenShift layers)                                                                                                                                                                                                                           | Layer to connect to (or provision the OpenShift environment). By default this automation assumes an existing OpenShift cluster will be used but if one of the options to provision OpenShift from above is selected then it should automatically be wired in. | 10 min |
...
4. You will need to populate these values. If you would like to use an external Git server, provide your username and your Personal Access Token to `TF_VAR_gitops_repo_username` and `TF_VAR_gitops_repo_token`. If not provided, a Git server (Gitea) will be provisioned on the cluster and used.
...
2. Next we need to set up the working directory for the automation:
   ...
   ```shell
   ./setup-workspace.sh [-p {cloud provider}] [-s {storage}] [-n {prefix name}] [-x {portworx spec file}] [-e {UDS email}] [-l {UDS last name}] [-u {UDS first name}] [-f {license file}] [-i {host id}]
   ```
   ...
   - **UDS email** - the email address for the UDS contact
   - **UDS first name** - the first name of the UDS contact
   - **UDS last name** - the last name of the UDS contact
   - **license file** - (optional) the full path to the file containing the license
   - **host id** - (optional) the host id associated with the license. This value is required if the license file is provided
...
3. The `setup-workspace.sh` script configures the `maximo.tfvars` and `gitops.tfvars` files with reasonable defaults. There are no other changes required in order to run the automation. If you would like to customize the configuration, see [advanced configuration options](#advanced-config).
...
| config_banner_text          | Banner text for the cluster console                                                                                                                              | Maximo Application Suite                              |
| portworx_spec_file          | The name of the file containing the portworx configuration spec yaml                                                                                             | portworx_essentials.yaml                              |

1. Update the desired values in `maximo.tfvars`
2. Save the `maximo.tfvars` file
